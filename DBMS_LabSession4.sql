...
SELECT 
cust.CUS_GENDER as CustomerGender
, count(cust.CUS_GENDER) as count 
 from customer as cust
 inner join `order` as ordr on ordr.CUS_ID = cust.CUS_ID
...
-- if any like if rating >4 then "Genuine Supplier" if rating >2 “Average Supplier” else “Supplier should not be considered”.

USE `order_directory`;
DROP procedure IF EXISTS `suppier_rating_verdict`;

DELIMITER $$
USE `order_directory`$$
CREATE PROCEDURE `suppier_rating_verdict` ()
BEGIN
